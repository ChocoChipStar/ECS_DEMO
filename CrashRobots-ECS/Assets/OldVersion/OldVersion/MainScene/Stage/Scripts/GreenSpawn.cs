...
public class GreenSpawn : MonoBehaviour
{
    ...
    //オブジェクトプレハブ
    [SerializeField]
    private ScoreManager scoreManeger = new ScoreManager();
    ...
    private float mww = 1.8f; // 壁との余白(margin with wall)
    ...
    private bool dis { get; set; } = false;
    ...
    // スポーンさせる
    private void GreenSpownInstatiate(Vector3 pos)
    {
        ...
        if (probability <= 7)
        {
            ...
            GameObject trash = Instantiate(microwavePrefab, pos, Quaternion.Euler(0, rotateY, -90));
        }
        else
        {
            pos.y = 1.5f;
            ...
            //var trashRotation = trash.transform.rotation.eulerAngles;
            //trashRotation.y = rotateY;
            //trash.transform.rotation = Quaternion.Euler(trashRotation);
        }
    }
}
